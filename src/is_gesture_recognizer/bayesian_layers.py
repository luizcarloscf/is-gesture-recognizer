...
class GaussianDistribution(object):
    ...
    def sample(self, store):
        ...
        if store:  # log posterior  - log prior
            self._cum_kl_div += (self.log_prob(self._last_sample).mean() -
                                 self._prior.log_prob(self._last_sample)).mean()
    ...
    def log_prob(self, sample):
        ...
        return -(
            (sample - self.mu)**2) / (2 * self.var) - log_scale - math.log(math.sqrt(2 * math.pi))
...
class LSTM(nn.Module):
    def __init__(self,
        ...
        for layer in range(num_layers):
            ...
            self._dense_layers_input.append(
                Linear(layer_input_size,
                       gate_size,
                       mu=mu,
                       logstd1=logstd1,
                       logstd2=logstd2,
                       ...
                       dropout=dropout,
                       type=type))
            self._dense_layers_hidden.append(
                Linear(hidden_size,
                       gate_size,
                       mu=mu,
                       logstd1=logstd1,
